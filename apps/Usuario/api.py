...
def usuario_api_view(request):
    ...
    if request.method == 'GET':
        ...
        usuario_serializer = UsuarioSerializer(
            usuario, many=True)  # Serializar los datos
...
def usuario_detail_api_view(request, pk=None):
    ...
    if usuario:
        ...
        if request.method == 'GET':
            usuario_serializer = UsuarioSerializer(
                usuario)  # Serializar los datos del usuario
            # Respuesta con los datos del usuario
            return Response(usuario_serializer.data, status=status.HTTP_200_OK)
        # Actualizar usuario
        elif request.method == 'PUT':
            usuario_serializer = UsuarioSerializer(
                usuario, data=request.data)  # Serializar los datos
            if usuario_serializer.is_valid():  # Validar los datos
        ...
        # Eliminar usuario
            ...
            try:
                ...
                # Manejo de errores
                return Response({'message': '¡No es posible eliminar un usuario en uso!'}, status=status.HTTP_409_CONFLICT)
    # Si no se encuentra el usuario
    # Manejo de errores
    return Response({'message': 'No se encontró el usuario indicado'}, status=status.HTTP_400_BAD_REQUEST)
